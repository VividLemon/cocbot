...
export default {
  data: new SlashCommandBuilder()
    .setName('clan-league-group')
    .setDescription('Gets the current clan league group')
  ...
  async execute (interaction: CommandInteraction): Promise<void> {
    ...
    const info = await cocClient.getClanWarLeagueGroup(clanTag)
    if (info == null) {
      return await interaction.reply({ content: i18n.__('noWarLeagueGroupInfo', { tag: clanTag }), ephemeral: true })
    }
    const file = await buildTempFile(JSON.stringify(info, null, 2))
    const mFile = new MessageAttachment(file)
    await interaction.reply({ content: i18n.__('warLeagueGroupInfo', { tag: clanTag }), ephemeral, attachments: [mFile] })
    unlink(file)
      .catch((error) => {
